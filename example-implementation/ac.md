...
### Senior Capabilities Skills

| Skill | Description | Primary Use | Required Before |
|-------|-------------|-------------|-----------------|
| `architecture-tradeoff-analysis` | Analyze architectural decisions with explicit tradeoffs, consequences, and alternatives. Use when designing systems, making technology choices, or evaluating design alternatives. | Architecture decisions | Before `openspec-new-change` |
| `requirement-negotiation` | Negotiate conflicting requirements and manage stakeholder expectations. Use when requirements conflict, stakeholders disagree, or scope needs to be managed. | Scope management | Before `openspec-new-change` |
| `boundary-enforcement` | Enforce architectural boundaries and prevent cross-layer dependencies. Use when implementing features that might violate architectural layers. | Architecture integrity | Before `openspec-apply-change` |
| `ownership-checkpoint` | Establish ownership and accountability checkpoints during development. Use when starting a new feature, before committing code, or when taking responsibility. | Accountability | Before `openspec-verify-change` |
| `decision-framework` | Make decisions with incomplete information and manage uncertainty. Use when facing decisions without full requirements or when choosing between multiple valid options. | Decision making | When ambiguity detected |
| `stakeholder-communication` | Communicate effectively with different audiences (technical and non-technical). Use when preparing updates for stakeholders or explaining technical decisions. | Communication | Before `openspec-archive-change` |
...
└─────────────────────────────────────────────────────────────────────────────────┘
    ...
    ║      └─ Analyze tradeoffs for key decisions  ║
    ║  12. requirement-negotiation                 ║
    ║      └─ Resolve conflicts before proceeding  ║
    ║  13. openspec-new-change                     ║
    ║      └─ Create proposal                      ║
    ║  14. microtask-decomposition                 ║
    ║      └─ ONLY if task is still too complex    ║
    ║      └─ Break into MICROTASKS (1 file each)  ║
    ║  15. openspec-continue-change                ║
    ║      └─ Draft Specs, Design, Tasks           ║
    ║  16. spec-analysis                           ║
    ║      └─ Verify consistency                   ║
    ║  17. requirement-checklist                   ║
    ║      └─ "Unit test" the specs                ║
    ║  18. api-design-principles [IF APIs]         ║
    ║      └─ Design REST/GraphQL APIs             ║
    ║  19. interface-design [IF UI]                ║
...
└─────────────────────────────────────────────────────────────────────────────────┘
    ...
    ║     └─ CLARIFY requirements first            ║
    ║  8. architecture-tradeoff-analysis           ║
    ║     └─ Analyze tradeoffs                     ║
    ║  9. requirement-negotiation                  ║
    ║     └─ Resolve conflicts                     ║
    ║  10. openspec-new-change                     ║
    ║      └─ Initialize change artifact           ║
    ║  11. microtask-decomposition                 ║
    ║      └─ If task too complex                  ║
    ╚════════════════════╬═════════════════════════╝
                         ║  CHECKPOINT
                         ▼
    ...
    ║  18. boundary-enforcement                    ║
    ║      └─ Verify architecture integrity        ║
    ║  19. decision-framework                      ║
    ║      └─ Document decisions under uncertainty ║
    ║  20. openspec-apply-change                   ║
    ║      └─ Implement code                       ║
    ║  21. testing-qa                              ║
    ║      └─ Automate test maintenance            ║
    ║  22. code-review                             ║
    ║      └─ Review for style/security/arch       ║
    ║  23. secure-coding-cybersecurity             ║
    ║      └─ Audit new code                       ║
    ...
    ║      └─ Fix regressions                      ║
    ║  27. ownership-checkpoint                    ║
    ║      └─ Take accountability                  ║
    ║  28. stakeholder-communication               ║
    ║      └─ Prepare updates                      ║
    ║  29. openspec-verify-change                  ║
    ║      └─ Final verification                   ║
    ║  30. documentation                           ║
    ║      └─ Generate technical docs & diagrams   ║
    ║  31. sync-index (IMPORTANT)                  ║
    ║      └─ Update docs with new changes         ║
    ║  32. changelog-generator                     ║
    ║      └─ Generate release notes               ║
    ║  33. ci-deploy                               ║
    ║      └─ Deploy and verify solution           ║
    ║  34. openspec-archive-change                 ║
    ║      └─ Archive change                       ║
    ╚══════════════════════════════════════════════╝
```
